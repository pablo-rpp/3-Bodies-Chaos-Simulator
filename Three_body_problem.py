...
def Bosco():
    ...
    for j in tqdm(range(RK.numIter)):
        RK.wrapKL()
        #k1, l1
        RK.kCalculus(0, nullIncrement)
        RK.lCalculus(0, nullIncrement)
        #k2, l2
        RK.kCalculus(1, 0.5*Rk.h*self.wrappedK)
        RK.lCalculus(1, 0.5*Rk.h*self.wrappedL)
        #k3, l3
        RK.kCalculus(2, 0.5*Rk.h*self.wrappedK)
        RK.lCalculus(2, 0.5*Rk.h*self.wrappedL)
        #k4, l4
        RK.kCalculus(3, (Rk.h-1)*self.wrappedK)
        RK.lCalculus(3, (Rk.h-1)*self.wrappedL)
